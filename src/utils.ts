...
export const updateShape = async (
    ...
    const orderByPos = [...orderedByType].sort(
    ...
    if(
        (bottomestChild && bottomestChild.bounds && bottomestChild.bounds.y)
        &&
        ...
        if(toppestChild.y !== bottomestChild.y){
            const centerOfChilds = (
                (
                    bottomestChild.bounds.bottom - toppestChild.bounds.top
                )/2 + toppestChild.bounds.top
            );
...
export const createFieldShape = async (
    params:{
        x:number,
        y:number,
        el?:HTMLElement,
