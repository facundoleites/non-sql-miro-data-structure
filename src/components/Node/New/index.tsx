...
import { getChildShapes, createCollectionShape, createDocumentShape, createFieldShape } from '../../../utils';
...
const NodeNewBase:React.FC<NewNodeType> = (
    {
    ...
    const [name,setname] = React.useState(
        isField?
            parent.metadata[APP_ID].fieldName as string
        :
            parent.plainText
    );
    ...
    const [fieldType,setfieldType] = React.useState(
        isField?
            parent.metadata[APP_ID].fieldType as string
        :
            ''
    );
    ...
    return(
        <article className={classes.join(' ')}>
            ...
            {
                childs.map(
                    thisChild=><EditNodeCard
                        key={`child_key_${thisChild.id}`}
                        node={thisChild}
                    />
                )
            }
...
const EditNodeCardBase:React.FC<EditNodeCardType> = (
    {
        node
    }
) => {
    ...
    const [fieldType,setfieldType] = React.useState(
        isField?
        node.metadata[APP_ID].fieldType as string
        :
            ''
    );
    const typeName = React.useMemo(
...
const AddChildrenNodeCardBase:React.FC<AddChildrenNodeCardType> = (
    ...
    const [type,settype] = React.useState<'field'|'document'|'collection'|'database'>('field');
    const isField = React.useMemo(
        ()=>type === 'field',
        [type]
    );
